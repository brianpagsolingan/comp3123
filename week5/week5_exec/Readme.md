...
express.Router() lets you group related routes together in Express.js. This keeps code neat and easier to manage, instead of puttign all routes in one file.
It also lets you add middleware to only certain routes, which makes your app more oraganized.
...
In Express, you handle errors by using try-catch in routes and a special error-handling middleware. If something goes wrong like a muissing file, you can send the error to the middleware with next(err). The middleware then sends a safe error respones instead of crashing the app
